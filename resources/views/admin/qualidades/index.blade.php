...
@section('content')
    <div class="admin-grid">
        <div class="usercard">
            <div class="utable__head" style="grid-template-columns: 1fr 92px;">
                <span>Descrição</span><span class="r">Ações</span>
            </div>

            @forelse ($qualidades as $q)
                <div class="utable__row" style="grid-template-columns: 1fr 92px; position:static; align-items:center;">
                    <form method="POST" action="{{ route('admin.qualidades.update', $q) }}" style="margin:0; display:flex; gap:8px; align-items:center;">
                        @csrf @method('PUT')
                        <input type="text" name="descricao" value="{{ $q->descricao }}" required
                               style="height:38px; font-size:13.5px;">
                        <button type="submit" class="mini" title="Salvar alteração">Salvar</button>
                    </form>
